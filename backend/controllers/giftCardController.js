...
const addListing = async (req, res) => {
  try {
    const { brand, balance, code, expiry } = req.body;
    ...
    const listing = new GiftCardListing({ brand, balance, code, expiry });
...
const getListings = async (req, res) => {
  try {
    const listings = await GiftCardListing.find().sort({ createdAt: -1 });
